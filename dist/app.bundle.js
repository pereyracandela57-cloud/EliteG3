const n2={apiKey:"AIzaSyAcrVq7ryWv5EJbRhZ1TmhFia-LAV10cN4",authDomain:"eliteg3-1650f.firebaseapp.com",databaseURL:"https://eliteg3-1650f-default-rtdb.firebaseio.com/",projectId:"eliteg3-1650f",storageBucket:"eliteg3-1650f.firebasestorage.app",messagingSenderId:"351711372153",appId:"1:351711372153:web:60ce4742396e37974f85a8",measurementId:"G-HGYPLEY6DJ"};if(!firebase.apps.length)firebase.initializeApp(n2);const w=firebase.database(),p2=firebase.storage(),{useState:E,useEffect:r,useMemo:f,useRef:zJ}=React,i2=(_,H="")=>{const U=String(_||"uploads").replace(/^\/+|\/+$/g,""),R=String(H||"archivo").trim().replace(/[^a-zA-Z0-9._-]/g,"_"),F=Date.now(),I=Math.random().toString(36).slice(2,10);return`${U}/${F}-${I}-${R||"archivo"}`},xQ=async(_,H="uploads")=>{if(!_)throw new Error("Archivo inválido.");const U=i2(H,_.name||"archivo"),R=p2.ref(U);await R.put(_);return R.getDownloadURL()};if(typeof window!=="undefined")window.uploadLocalFileToStorage=xQ;const $J=["C","P","B","N","S","E","X","R"],l2=["R"],_Q="anonimo/galeria",SJ="__anonimo_gallery__",t2=["PERSONAJE","ETIQUETA","GENERAL"],V1={PERSONAJE:"Personaje",ETIQUETA:"Etiqueta",GENERAL:"General"},lJ=[{id:"perfil",label:"Perfil",labels:[]},{id:"colaPiernas",label:"Cola/Piernas",labels:["C"]},{id:"pechos",label:"Pechos",labels:["P"]},{id:"cuerpoCintura",label:"Cintura/Cuerpo",labels:["N","B"]},{id:"sensualidad",label:"Sensualidad",labels:["E","S"]}],o2={perfil:"perfil",pecho:"pechos",pechos:"pechos",cola:"colaPiernas",piernas:"colaPiernas",cuerpo:"cuerpoCintura",cintura:"cuerpoCintura",sensualidad:"sensualidad"},iZ=()=>lJ.reduce((_,H)=>{_[H.id]="";return _},{}),WJ=(_={})=>{const H=iZ(),U=_&&typeof _==="object"?_:{};return Object.keys(H).reduce((R,F)=>{R[F]=typeof U[F]==="string"?U[F].trim():"";return R},{...H})},EZ=(_="")=>lJ.find((H)=>H.id===_),a2=(_="")=>o2[(_||"").trim().toLowerCase()]||"",VZ={C:{bg:"linear-gradient(135deg, rgba(34,211,238,0.92), rgba(14,116,144,0.95))",border:"rgba(103,232,249,0.95)",glow:"rgba(34,211,238,0.8)",text:"#ecfeff"},P:{bg:"linear-gradient(135deg, rgba(244,114,182,0.92), rgba(157,23,77,0.95))",border:"rgba(251,207,232,0.95)",glow:"rgba(244,114,182,0.78)",text:"#fff1f2"},B:{bg:"linear-gradient(135deg, rgba(251,191,36,0.94), rgba(180,83,9,0.95))",border:"rgba(253,224,71,0.98)",glow:"rgba(251,191,36,0.82)",text:"#fffbea"},N:{bg:"linear-gradient(135deg, rgba(163,230,53,0.92), rgba(63,98,18,0.95))",border:"rgba(217,249,157,0.95)",glow:"rgba(163,230,53,0.78)",text:"#f7fee7"},S:{bg:"linear-gradient(135deg, rgba(192,132,252,0.92), rgba(107,33,168,0.95))",border:"rgba(233,213,255,0.95)",glow:"rgba(192,132,252,0.8)",text:"#faf5ff"},E:{bg:"linear-gradient(135deg, rgba(248,113,113,0.92), rgba(153,27,27,0.95))",border:"rgba(254,202,202,0.95)",glow:"rgba(248,113,113,0.8)",text:"#fef2f2"},X:{bg:"linear-gradient(135deg, rgba(239,68,68,0.98), rgba(127,29,29,0.98))",border:"rgba(254,226,226,1)",glow:"rgba(239,68,68,0.95)",text:"#fff7f7"},R:{bg:"linear-gradient(135deg, rgba(148,163,184,0.95), rgba(71,85,105,0.98))",border:"rgba(226,232,240,0.95)",glow:"rgba(148,163,184,0.82)",text:"#f8fafc"},DEFAULT:{bg:"linear-gradient(135deg, rgba(51,65,85,0.92), rgba(15,23,42,0.95))",border:"rgba(148,163,184,0.85)",glow:"rgba(148,163,184,0.45)",text:"#f8fafc"}},jQ=(_,H)=>{if(!H)return 0;return(_+H)%H},U1=(_,H,U=!1)=>{const R=Array.isArray(H)?H.length:0;if(!R)return null;if(R===1)return 0;if(U){const F=H.map((I,p)=>({item:I,idx:p})).filter(({item:I,idx:p})=>p!==_&&I&&I.url).map(({idx:I})=>I);if(!F.length)return jQ((_??0)+1,R);return F[Math.floor(Math.random()*F.length)]}return jQ((_??0)+1,R)},tJ=(_)=>VZ[_]||VZ.DEFAULT,UZ=(_,H=!1)=>{const U=_==="INICIAL"?VZ.DEFAULT:_==="100"?VZ.B:tJ(_);return{"--btn-neon-color":U.glow,borderColor:U.border,color:U.text,boxShadow:H?`inset 0 1px 0 rgba(186,230,253,0.35), 0 0 0 1px ${U.border}, 0 0 20px ${U.glow}`:`inset 0 1px 0 rgba(186,230,253,0.2), 0 0 0 1px color-mix(in srgb, ${U.border} 65%, rgba(15,23,42,0.65) 35%), 0 0 12px color-mix(in srgb, ${U.glow} 60%, transparent)`,textShadow:`0 0 8px color-mix(in srgb, ${U.glow} 70%, transparent)`,filter:H?"brightness(1.14)":"brightness(1)"}},D1=/\.(mp4|webm|ogg|mov|m4v)(\?.*)?$/i,r2=/\.(mp3|wav|ogg|m4a|aac|flac)(\?.*)?$/i,e2=/\.gif(\?.*)?$/i,J3=/(?:youtube\.com\/(?:watch\?v=|embed\/|shorts\/)|youtu\.be\/)([A-Za-z0-9_-]{6,})/i,Q3=/vimeo\.com\/(?:video\/)?(\d+)/i,HQ=(_)=>{const H=(_||"").trim();if(!H)return null;const U=H.match(J3);if(U)return{provider:"youtube",src:`https://www.youtube.com/embed/${U[1]}`,title:"Video de YouTube"};const R=H.match(Q3);if(R)return{provider:"vimeo",src:`https://player.vimeo.com/video/${R[1]}`,title:"Video de Vimeo"};return null},C1="eliteg3_cache_perfiles_v1",R1="eliteg3_cache_anon_gallery_v1",Z3=["eporner.com"],B1=(_="")=>{const H=String(_||"").trim();if(!H||H.startsWith("data:")||H.startsWith("blob:"))return!1;try{const R=new URL(H).hostname.toLowerCase();return Z3.some((F)=>R===F||R.endsWith(`.${F}`))}catch{return!1}},qJ=(_="",H="")=>{const U=String(_||"").trim();if(!U||B1(U))return H;return U},oJ=(_="",H="")=>{if(H==="video"||H==="image")return H;const U=(_||"").trim();if(D1.test(U)||HQ(U))return"video";return"image"},lZ=(_="")=>{return typeof _==="string"?_.trim():""},e=(_,H="")=>{if(typeof _==="string")return{url:qJ(_.trim(),""),label:"",type:oJ(_,H),autor:""};if(_&&typeof _==="object"){const U=qJ((_.url||"").trim(),"");return{url:U,label:$J.includes(_.label)?_.label:"",type:oJ(U,_.type||H),autor:lZ(_.autor)}}return{url:"",label:"",type:oJ("",H),autor:""}},$3=(_)=>e(_).url,T1=(_)=>e(_).label,aJ=(_)=>e(_).type,tZ=(_,H,U="image")=>Array.isArray(_?.[H])?_[H].map((R)=>e(R,U)).filter((R)=>R.url):[],X3=(_={})=>({firebaseId:SJ,nombre:"Anónimo",nacionalidad:"",ciudad:"",profesion:"Galería",fechaNacimiento:"",estaturaCm:"",fotos:[],galeria:{fotos:tZ(_,"fotos","image"),gifs:tZ(_,"gifs","image"),videos:tZ(_,"videos","video")},batallaFotosPreferidas:iZ(),puntuaciones:PQ(),isAnonymousGallery:!0}),Y3=(_={})=>{if(_?.isAnonymousGallery||_?.firebaseId===SJ)return"anonimo";return _?.firebaseId||""},q3=(_={})=>{const H=qJ(String(_?.fotos?.[0]||"").trim(),""),U=WJ(_?.batallaFotosPreferidas||_?.galeria?.battlePhotoPreferences||{});if(!H)return!1;return lJ.filter((R)=>R.id!=="perfil").every((R)=>Boolean(String(U?.[R.id]||"").trim()))},_3=(_={})=>{const H=[],U=qJ(String(_?.fotos?.[0]||"").trim(),""),R=WJ(_?.batallaFotosPreferidas||_?.galeria?.battlePhotoPreferences||{});if(!U)H.push("Perfil");lJ.filter((F)=>F.id!=="perfil").forEach((F)=>{if(!String(R?.[F.id]||"").trim())H.push(F.label)});return H},j3=(_=null)=>{if(!_)return;const H=window.open("","_blank");if(!H)return;const U=_?.firebaseId||_?.id||"",R=Array.isArray(_?.galeria?.fotos)?_.galeria.fotos.map((z)=>e(z,"image")).filter((z)=>z.url).map((z,G)=>({...z,sourceTag:"fotos",sourceIndex:G})):[],F=R.filter((z)=>z.type==="image"&&z.sourceTag==="fotos"),I=WJ(_?.batallaFotosPreferidas||_?.galeria?.battlePhotoPreferences||{}),p=qJ(String(_?.fotos?.[0]||_?.foto||"").trim(),""),XJ=R.filter((z)=>z.type==="image"&&z.url).reduce((z,G)=>{z[G.url]=G;return z},{}),t=R.length?R.map((z,G)=>`
                    <button
                        type="button"
                    ...
                    >
                        <div class="multimedia-thumb-wrap">
                            <img src="${z.url}" alt="Multimedia ${G+1}" style="width:100%;height:100%;object-fit:contain;" loading="lazy" />
                            <span class="multimedia-thumb-label">${z.label||"SIN ETIQUETA"}</span>
                        </div>
                    </button>
                `).join(""):'<p class="text-slate-300">Sin contenido en galería.</p>',b=F.length?F.map((z,G)=>`
                    <button
                        type="button"
                    ...
                    >
                        <div class="multimedia-thumb-wrap multimedia-thumb-wrap--broken">
                            <img src="${z.url}" alt="Imagen rota ${G+1}" style="width:100%;height:100%;object-fit:contain;" loading="lazy" />
                            <span class="multimedia-thumb-label">${z.label||"SIN ETIQUETA"}</span>
                        </div>
                    </button>
                `).join(""):'<p class="text-slate-300">Sin imágenes en galería.</p>',V=lJ.map((z)=>{const G=z.id==="perfil",P=G?p:I[z.id]||"",K=P?G?{url:P}:XJ[P]||{url:P,label:"",sourceIndex:-1}:null,VJ=Boolean(P);return`
                    <div class="multimedia-slot-card ${VJ?"is-assigned":"is-missing"}" data-slot-id="${z.id}">
                        ...
                        <div class="multimedia-slot-preview">
                            ${VJ?`<img src="${K.url}" alt="${z.label}" loading="lazy" />`:'<span class="multimedia-slot-empty">Sin foto designada</span>'}
                        </div>
                        <div class="multimedia-slot-actions">
                            ${!G?`<button type="button" class="multimedia-slot-assign-btn" data-slot-assign="${z.id}">DESIGNAR FOTO</button>`:""}
                ...
                <html lang="es">
                    ...
                    <body class="text-slate-200">
                        ...
                        <script>
                            ...
                            const slotConfigById = ${JSON.stringify(lJ.reduce((z,G)=>{z[G.id]=G;return z},{}))};
            ...
            `);H.document.close()},H3=({isOpen:_=!1,profile:H=null,onClose:U=()=>{}})=>{const[R,F]=E(!0),[I,p]=E(!0);r(()=>{if(!_)return;F(!0);p(!0)},[_,H?.firebaseId]);if(!_||!H)return null;const XJ=Array.isArray(H?.galeria?.fotos)?H.galeria.fotos.map((b)=>e(b,"image")).filter((b)=>b.url):[],t=Object.entries(H?.puntuaciones||{}).map(([b,V])=>({label:b,value:Number(V)||0})).sort((b,V)=>V.value-b.value).slice(0,5);return React.createElement("div",{className:"fixed inset-0 z-[180] flex items-center justify-center bg-slate-950/80 backdrop-blur-sm px-4 py-8",onClick:U,role:"presentation"},React.createElement("section",{className:"gothic-frame surface-panel surface-panel--neon rounded-[2rem] p-6 md:p-8 w-full max-w-5xl max-h-[90vh] overflow-y-auto relative",onClick:(b)=>b.stopPropagation(),role:"dialog","aria-modal":"true","aria-label":"Multimedia del personaje"},React.createElement("button",{type:"button",onClick:U,className:"absolute top-4 right-4 w-10 h-10 rounded-full border border-cyan-200/35 bg-slate-900/80 text-slate-100"},"✕"),React.createElement("h2",{className:"font-title text-center text-3xl md:text-4xl text-white uppercase tracking-wide"},"Multimedia"),React.createElement("p",{className:"text-center text-cyan-100/80 text-xs uppercase tracking-[0.2em] mt-2"},H?.nombre||"Personaje"),React.createElement("div",{className:"mt-6 space-y-4"},React.createElement("article",{className:"surface-panel rounded-2xl border border-cyan-200/20"},React.createElement("button",{type:"button",onClick:()=>F((b)=>!b),className:"w-full px-4 py-3 text-left font-black uppercase tracking-wide flex items-center justify-between"},React.createElement("span",null,"Galería"),React.createElement("span",null,R?"−":"+")),R&&React.createElement("div",{className:"px-4 pb-4 grid grid-cols-2 md:grid-cols-4 gap-3"},XJ.length?XJ.map((b,V)=>React.createElement("div",{key:`${b.url}-${V}`,className:"rounded-xl overflow-hidden border border-slate-600/40 bg-slate-900/40"},React.createElement("img",{src:b.url,alt:`Multimedia ${V+1}`,className:"w-full h-36 object-cover",loading:"lazy"}))):React.createElement("p",{className:"text-sm text-slate-300 col-span-full"},"Sin contenido en galería."))),React.createElement("article",{className:"surface-panel rounded-2xl border border-cyan-200/20"},React.createElement("button",{type:"button",onClick:()=>p((b)=>!b),className:"w-full px-4 py-3 text-left font-black uppercase tracking-wide flex items-center justify-between"},React.createElement("span",null,"5 Principales"),React.createElement("span",null,I?"−":"+")),I&&React.createElement("div",{className:"px-4 pb-4 space-y-2"},t.length?t.map((b)=>React.createElement("div",{key:b.label,className:"surface-panel rounded-xl px-3 py-2 flex items-center justify-between"},React.createElement("span",{className:"text-sm text-slate-200 uppercase"},b.label),React.createElement("strong",{className:"text-cyan-200"},b.value))):React.createElement("p",{className:"text-sm text-slate-300"},"Sin puntajes cargados."))))))},A1=async(_="",{timeoutMs:H=12000,retries:U=1}={})=>{const R=String(_||"").trim();if(!R)return!0;const F=()=>new Promise((I)=>{const p=new Image;let XJ=!1;const t=(V)=>{if(XJ)return;XJ=!0;p.onload=null;p.onerror=null;I(Boolean(V))},b=window.setTimeout(()=>t(!0),H);p.onload=()=>{clearTimeout(b);const V=Number(p.naturalWidth)>0&&Number(p.naturalHeight)>0;t(!V)};p.onerror=()=>{clearTimeout(b);t(!0)};p.src=R});for(let I=0;I<=U;I+=1)if(!await F())return!1;return!0},z1=(_,H)=>{const U=(H||"").trim().toLowerCase(),R=Array.isArray(_?.galeria?.fotos)?_.galeria.fotos.map((P)=>e(P,"image")).filter((P)=>P.type==="image"&&P.url):[],I=qJ(_?.fotos?.[0],"")||"https://via.placeholder.com/400x500";if(new Set(["rostro","ojos","boca","cabello","carisma","elegancia","dulzura","talento","estatura"]).has(U))return I;const XJ=(P=[])=>{const K=R.filter((c)=>P.includes(T1(c)));if(!K.length)return"";const VJ=Math.floor(Math.random()*K.length);return K[VJ]?.url||""},t=a2(U),b=EZ(t);if(!b)return I;const z=WJ(_?.batallaFotosPreferidas)[t];if(z){if(R.some((K)=>K.url===z))return z}return XJ(b.labels)||I},W3=(_="")=>e2.test((_||"").trim()),WQ="data:image/svg+xml;charset=UTF-8,%3Csvg%20xmlns%3D%27http%3A//www.w3.org/2000/svg%27%20viewBox%3D%270%200%20256%20256%27%3E%3Crect%20width%3D%27256%27%20height%3D%27256%27%20rx%3D%2740%27%20fill%3D%27%23020617%27/%3E%3Ctext%20x%3D%27128%27%20y%3D%27152%27%20text-anchor%3D%27middle%27%20font-size%3D%27120%27%3E%F0%9F%98%A2%3C/text%3E%3C/svg%3E",oZ=`if(this.dataset.fallbackApplied==='true')return;this.dataset.fallbackApplied='true';this.src='${WQ}';this.style.objectFit='contain';this.style.padding='12px';this.style.background='#020617';this.alt='Imagen no disponible';`,xJ=(_)=>{const H=_?.currentTarget||_?.target;if(!H||H.dataset.fallbackApplied==="true")return;H.dataset.fallbackApplied="true";H.src=WQ;H.style.objectFit="contain";H.style.padding="12px";H.style.background="#020617";H.alt="Imagen no disponible"},z3="g2_elite_database_v4",N3="g2_elite_categories_v4",rJ=["Rostro","Ojos","Boca","Cabello","Cuerpo","Cola","Pechos","Cintura","Piernas","Estatura","Sensualidad","Carisma","Elegancia","Dulzura","Talento"],aZ=[...rJ],rZ=[{id:"PROFESIONES",label:"Profesiones",description:"Compará perfiles por profesión."},{id:"NACIONALIDADES",label:"Nacionalidades",description:"Compará perfiles por nacionalidad."},{id:"EDADES",label:"Edades",description:"Compará perfiles por rango etario."},{id:"GENERAL",label:"General",description:"Modo libre sin segmentación extra."}],O3=rZ.map((_)=>_.id),OQ=(_="")=>{return rZ.find((U)=>U.id===_)?.label||"General"},F3=(_="")=>{const H=String(_||"").trim();if(!H)return"";return encodeURIComponent(H.toLowerCase())},L3=(_="")=>{const H=String(_||"").trim();if(!H)return"";try{return decodeURIComponent(H).toLowerCase()}catch{return H.toLowerCase()}},FQ=(_,H="GENERAL",U="")=>{const R=String(_||"").trim(),F=String(H||"GENERAL").trim().toUpperCase(),I=F3(U);if(!R)return"";return`${F}::${I||"all"}::${R}`},DJ=(_="")=>String(_||"").trim(),N1=(_="")=>{const H=String(_||"").split("::");if(H.length<3)return{scopeId:"GENERAL",groupKey:"all",arenaName:""};const U=H.slice(2).join("::");return{scopeId:String(H[0]||"GENERAL").trim().toUpperCase()||"GENERAL",groupKey:L3(H[1]||"all")||"all",arenaName:String(U||"").trim()}},PQ=()=>rJ.reduce((_,H)=>{_[H]=0;return _},{}),M1=[],eZ={Cantante:{glyph:"crown",color:"rgba(34, 211, 238, 0.8)",tailwind:"cyan-400"},Actriz:{glyph:"theater",color:"rgba(34, 197, 94, 0.8)",tailwind:"green-500"},Modelo:{glyph:"floron",color:"rgba(168, 85, 247, 0.8)",tailwind:"purple-500"},NSFW:{glyph:"torch",color:"rgba(239, 68, 68, 0.8)",tailwind:"red-500"},Influencer:{glyph:"sigil",color:"rgba(236, 72, 153, 0.8)",tailwind:"pink-500"},"Bailarina y Atleta":{glyph:"floron",color:"rgba(249, 115, 22, 0.8)",tailwind:"orange-500"},Otro:{glyph:"sigil",color:"rgba(148, 163, 184, 0.8)",tailwind:"slate-400"}},M3=(_="")=>{const H=String(_||"").toLowerCase();if(H.includes("cantante"))return"crown";if(H.includes("nsfw"))return"torch";if(H.includes("actriz")||H.includes("actor"))return"theater";if(H.includes("modelo"))return"floron";if(H.includes("influencer"))return"sigil";if(H.includes("periodista"))return"quill";if(H.includes("bailarina"))return"floron";if(H.includes("atleta"))return"shield";return"shield"},E3=(_="")=>{const H=String(_||"").toLowerCase();if(H.includes("cantante"))return"\uD83C\uDFA4";if(H.includes("nsfw"))return"\uD83D\uDD25";if(H.includes("actriz")||H.includes("actor"))return"\uD83C\uDFAC";if(H.includes("modelo"))return"\uD83D\uDC60";if(H.includes("influencer"))return"\uD83D\uDCF1";if(H.includes("periodista"))return"\uD83D\uDC53";if(H.includes("bailarina"))return"\uD83D\uDC83";if(H.includes("atleta"))return"\uD83C\uDFCB️‍♀️";return"\uD83D\uDCD6"},vQ=(_="")=>{const H=String(_||"").trim(),U=eZ[H]||eZ.Otro,R=String(U?.color||"rgba(148, 163, 184, 0.8)").replace(/,\s*[\d.]+\)$/,", 1)"),F=String(U?.color||"rgba(148, 163, 184, 0.8)").replace(/,\s*[\d.]+\)$/,", 0.75)");return{baseColor:R,glowColor:F}},K3=({variant:_="sigil",size:H=18,className:U="",tint:R="currentColor"})=>{const F={width:H,height:H,viewBox:"0 0 24 24",fill:"none",xmlns:"http://www.w3.org/2000/svg",className:U};if(_==="crown")return React.createElement("svg",{...F,stroke:R,strokeWidth:"1.7",strokeLinecap:"round",strokeLinejoin:"round"},React.createElement("path",{d:"M3 17h18v3H3z"}),React.createElement("path",{d:"M4 17l3-8 5 5 5-7 3 10"}));if(_==="shield")return React.createElement("svg",{...F,stroke:R,strokeWidth:"1.7",strokeLinecap:"round",strokeLinejoin:"round"},React.createElement("path",{d:"M12 3l7 3v6c0 4.3-2.4 7-7 9-4.6-2-7-4.7-7-9V6l7-3z"}),React.createElement("path",{d:"M9.5 11h5M12 8.5v5"}));if(_==="floron")return React.createElement("svg",{...F,stroke:R,strokeWidth:"1.6",strokeLinecap:"round",strokeLinejoin:"round"},React.createElement("path",{d:"M12 4v16"}),React.createElement("path",{d:"M6 9c1.4 0 2.8.9 3.4 2.2C8.2 11.5 7 12.8 7 14.3 5.4 13.5 4.3 11.9 4.3 10 4.3 9.5 5 9 6 9z"}),React.createElement("path",{d:"M18 9c-1.4 0-2.8.9-3.4 2.2 1.2.3 2.4 1.6 2.4 3.1 1.6-.8 2.7-2.4 2.7-4.3 0-.5-.7-1-1.7-1z"}),React.createElement("circle",{cx:"12",cy:"12",r:"2.2"}));if(_==="quill")return React.createElement("svg",{...F,stroke:R,strokeWidth:"1.7",strokeLinecap:"round",strokeLinejoin:"round"},React.createElement("path",{d:"M20 4c-7 1-11.5 5.2-13 12l4-1 1 4c6.8-2.4 8.4-8.4 8-15z"}),React.createElement("path",{d:"M8 16l-4 4"}));if(_==="torch")return React.createElement("svg",{...F,stroke:R,strokeWidth:"1.7",strokeLinecap:"round",strokeLinejoin:"round"},React.createElement("path",{d:"M9 13l6-6"}),React.createElement("path",{d:"M8 14l2 2 2-2-2-2z"}),React.createElement("path",{d:"M11 16l-4 5h10l-4-5"}),React.createElement("path",{d:"M15.5 4.5c.9 1.3.5 3.2-1 4.3-1.5-1.4-1.8-3.3-.8-4.9.6.1 1.2.3 1.8.6z"}));if(_==="theater")return React.createElement("svg",{...F,stroke:R,strokeWidth:"1.6",strokeLinecap:"round",strokeLinejoin:"round"},React.createElement("path",{d:"M5 6h14v11a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2z"}),React.createElement("path",{d:"M9 11h.01M15 11h.01"}),React.createElement("path",{d:"M8.5 15c1 .8 2.2 1.2 3.5 1.2s2.5-.4 3.5-1.2"}));return React.createElement("svg",{...F,stroke:R,strokeWidth:"1.7",strokeLinecap:"round",strokeLinejoin:"round"},React.createElement("circle",{cx:"12",cy:"12",r:"8.5"}),React.createElement("path",{d:"M12 6v12M6 12h12"}))},o=({name:_,size:H=20,className:U=""})=>{return React.createElement("i",{"data-lucide":_.toLowerCase().replace(/([a-z])([A-Z])/g,"$1-$2"),style:{width:H,height:H},className:`inline-block ${U}`})},J0=({targetWindow:_,profileName:H,profession:U,photos:R,editingId:F,battlePhotoPrefs:I={},profilePhotoUrl:p=""})=>{if(!_||_.closed)return;const XJ=WJ(I),t=qJ(String(p||"").trim(),""),b=(R||[]).map((V,z)=>{const G=e(V);return{...G,sourceTag:V?.sourceTag||(G.type==="video"?"videos":"fotos"),sourceIndex:typeof V?.sourceIndex==="number"?V.sourceIndex:z}}).filter((V)=>V.url);_.document.open();_.document.write(`
            <!DOCTYPE html>
            <html>
            <head>
                <title>Galería de ${H}</title>
                <style>
                    body {
                        background:
            ...
            <body>
                ...
                <details open style="width:100%; margin-bottom: 20px; border-radius: 12px; border: 1px solid rgba(148,163,184,0.28); background: rgba(2,6,23,0.45);">
                    ...
                    ${lJ.map((V)=>{const z=V.id==="perfil",G=z?t:XJ[V.id]||"",P=Boolean(G),K=!z;return`
                            <div class="gallery-slot-card" data-slot-id="${V.id}" style="border:1px solid ${P?"rgba(74,222,128,0.95)":"rgba(248,113,113,0.95)"}; border-radius:10px; padding:10px; background:${P?"linear-gradient(155deg, rgba(20,83,45,0.95) 0%, rgba(21,128,61,0.82) 45%, rgba(5,46,22,0.96) 100%)":"linear-gradient(155deg, rgba(127,29,29,0.96) 0%, rgba(185,28,28,0.84) 45%, rgba(69,10,10,0.96) 100%)"}; box-shadow: inset 0 2px 0 rgba(255,255,255,0.24), inset 0 -2px 0 rgba(2,6,23,0.45), inset 0 0 0 1px ${P?"rgba(187,247,208,0.25)":"rgba(254,202,202,0.22)"}, 0 8px 16px rgba(2,6,23,0.45), 0 0 18px ${P?"rgba(74,222,128,0.35)":"rgba(248,113,113,0.3)"}; position: relative;">
                                ...
                                <div style="margin-top:8px; border-radius:8px; overflow:hidden; aspect-ratio:4/3; border:1px dashed rgba(148,163,184,0.45); background: rgba(2,6,23,0.86); display:flex; align-items:center; justify-content:center;">
                                    ${P?`<img src="${G}" alt="Vista previa ${V.label}" onerror="${oZ}" style="width:100%; height:100%; object-fit:cover; display:block;" />`:'<span style="font-size:10px; color:#fca5a5; text-transform:uppercase; letter-spacing:0.08em; font-weight:700; text-align:center; padding:0 8px;">Sin foto designada</span>'}
                                </div>
                    ...
                    ${b.length?b.map((V,z)=>{const G=$3(V),P=T1(V),K=tJ(P),VJ={CANTANTE:{color:"#0ea5e9",sombra:"rgba(14,165,233,0.8)"},NSFW:{color:"#ef4444",sombra:"rgba(239,68,68,0.8)"},ACTRIZ:{color:"#22c55e",sombra:"rgba(34,197,94,0.8)"},MODELO:{color:"#f97316",sombra:"rgba(249,115,22,0.8)"},BAILARINA:{color:"#a855f7",sombra:"rgba(168,85,247,0.8)"},INFLUENCER:{color:"#ec4899",sombra:"rgba(236,72,153,0.8)"},PERIODISTA:{color:"#facc15",sombra:"rgba(250,204,21,0.8)"},ATLETA:{color:"#78350f",sombra:"rgba(120,53,15,0.8)"},OTRO:{color:"#ffffff",sombra:"rgba(255,255,255,0.8)"},DEFAULT:{color:"#334155",sombra:"transparent"}},c=VJ[U?.toUpperCase()]||VJ.DEFAULT;return`
                        <div
                            ...
                            style="
                                ...
                                cursor: ${aJ(V)==="image"?"grab":"pointer"};
                                position: relative;
                                background: #0f172a;
                            "
                        ...
                        >
                           <button
                            onclick="event.stopPropagation(); window.opener.postMessage({type: 'DELETE_IMAGE', index: ${V.sourceIndex}, mediaType: '${aJ(V)}', id: '${F}'}, '*');"
                            style="
                                position: absolute;
                                top: 5px;
                                right: 5px;
                            ...
                            type="button"
                        >✕</button>
                        ${aJ(V)==="video"?(()=>{const NJ=HQ(G);if(NJ)return`<div style="width: 100%; height: 100%; background: radial-gradient(circle at top, rgba(34,211,238,0.35), rgba(15,23,42,0.96)); display:flex; flex-direction:column; align-items:center; justify-content:center; gap:10px; padding: 18px; box-sizing:border-box; text-align:center;"><div style="width:64px; height:64px; border-radius:999px; border:1px solid rgba(34,211,238,0.5); background:rgba(15,23,42,0.75); display:flex; align-items:center; justify-content:center; color:#22d3ee; font-size:28px;">▶</div><div style="font-size:12px; font-weight:800; letter-spacing:0.12em; text-transform:uppercase; color:#e2e8f0;">Video embebido</div><div style="font-size:11px; color:#94a3b8; max-width:100%; word-break:break-word;">${NJ.provider.toUpperCase()}</div></div>`;return`<video src="${G}" style="width: 100%; height: 100%; object-fit: cover;" muted playsinline preload="metadata"></video>`})():`<img src="${G}" alt="Imagen de la galería" onerror="${oZ}" style="width: 100%; height: 100%; object-fit: cover;" />`}
                        <div style="position:absolute; left: 12px; top: 12px; z-index: 15; padding: 6px 10px; border-radius: 999px; background: rgba(2,6,23,0.72); border: 1px solid rgba(148,163,184,0.24); color: #e2e8f0; display:flex; align-items:center; justify-content:center; font-size: 10px; font-weight: 900; letter-spacing: 0.2em; text-transform: uppercase; backdrop-filter: blur(8px);">${aJ(V)==="video"?"VIDEO":"IMG"}</div>
